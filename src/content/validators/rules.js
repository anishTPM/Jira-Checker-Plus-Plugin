...
export const rules = [
  ...
  // Story Points (beyond New/Defined only)
  (fields) => {
  ...
  // Original Estimate for Sub-tasks
  (fields) => {
  ...
  (fields) => {
    if (!F.isType(fields, 'story') && !F.isType(fields, 'sub')) return null;
  ...
  // In Progress but no Sprint (beyond New/Defined)
  (fields) => {
    const type = F.issueType(fields);
